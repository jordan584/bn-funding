...
const ASSET_GAP = 16;
const ASSET_HEIGHT = ASSET_HEADER_HEIGHT + TABLE_HEADER_HEIGHT + VENUE_ROW_HEIGHT * 5;
...
function assetBlock(row: CompositeFundingRow, y: number): string {
  ...
  const output = [
    `<rect x="${left}" y="${y}" width="${width}" height="${ASSET_HEIGHT}" rx="14" fill="${COLORS.panel}" stroke="${COLORS.border}"/>`,
...
export function renderFundingReportSvg(
  ...
  const height = HEADER_HEIGHT + rows.length * (ASSET_HEIGHT + ASSET_GAP) + FOOTER_HEIGHT;
  ...
  return [
    ...
    ...rows.map((row, index) => assetBlock(row, HEADER_HEIGHT + index * (ASSET_HEIGHT + ASSET_GAP))),
